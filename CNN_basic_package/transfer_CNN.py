...
class TransferCNN:
    def __init__(self, preprocessing: Any) -> None:
        self.params = DataIO.load_config("preprocessing_inception", "training", "general")
        self.params["preprocessing"] = self.params.pop("preprocessing_inception")
        self.preprocessing = preprocessing

    def train_(self) -> None:
        base_model = InceptionV3(weights='imagenet', include_top=False, input_shape=tuple(self.params["preprocessing"].values()))
        ...
        checkpointing = ModelCheckpoint(
            DataIO.get_model_path("custom_transfer"), save_best_only=True, monitor="val_loss", mode="min"
        )
    ...
    def test_(self) -> None:
        model = load_model(DataIO.get_model_path("custom_transfer"))
